...
(function main() {
  ...
  const checkTweets = setInterval(() => {
    ...
    const Tweet = window.Tweet; // Declare the Tweet variable
    ...
    for (const r of valid)
      countsMap.set(r.type, (countsMap.get(r.type) ?? 0) + 1);
    ...
    document.querySelector("#numberActivities").textContent = String(
      new Set(valid.map((v) => v.type)).size
    );
    ...
    const avgByType = Array.from(aggType.entries()).map(
      ([type, { total, n }]) => ({ type, avg: total / n })
    );
    avgByType.sort((a, b) => b.avg - a.avg);
    document.querySelector("#longestActivityType").textContent =
      avgByType[0]?.type ?? "(n/a)";
    document.querySelector("#shortestActivityType").textContent =
      avgByType[avgByType.length - 1]?.type ?? "(n/a)";
    ...
    document.querySelector("#weekdayOrWeekendLonger").textContent =
      weAvg > wkAvg ? "weekends" : "weekdays";
    ...
    const vegaEmbed = window.vegaEmbed; // Declare the vegaEmbed variable
    ...
    const render = () => {
      ...
      document.querySelector("#aggregate").textContent = showingMeans
        ? "Show raw"
        : "Show means";
    };
    render();
